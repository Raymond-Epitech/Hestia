...
public class ExpenseService(ILogger<ColocationService> logger,
    ...
    IAppCache cache,
    IReceiptScannerService receiptScannerService) : IExpenseService
{
    /// <summary>
    ...
    public async Task<ReceiptScanResult> AddReceiptAsync(ReceiptInput input)
    {
        var stream = input.ReceiptPicture.OpenReadStream();
        var receiptExpense = await receiptScannerService.ScanReceiptAsync(stream, "image/jpeg");
        await realTimeService.SendToGroupAsync(input.ColocationId, "ReceiptAdded", receiptExpense);
    ...
    public async Task<bool> TriggerAllExpenseAutomation()
    {
        var automations = await expenseAutomationRepository.Query()
            .Where(e => e.DayOfTheMonth == DateTime.Now.Day)
            .Include(e => e.Expenses)
            .ToListAsync();

        if (automations is null || automations.Count == 0)
        {
            logger.LogInformation($"Info : No expense automations found");
            return false;
        }

        logger.LogInformation($"Succes : Expense automations found");
        ...
        foreach (var automation in automations)
        {
            ...
            logger.LogInformation($"Succes : Expense automation with id {automation.Id} triggered");
        }
