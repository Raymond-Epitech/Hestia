            ...
            <div class="modal-background" @click="handleClose">
                <div class="modal" @click.stop>
                    ...
                    <div class="center-container">
                        <div v-for="refound in refund_to" :key="refound.from" class="refound-rectangle from">
                            <text>{{ refound.fromUsername}} {{ $t('owes_you') }}</text>
                            <text class="number">
                                {{ refound.amount}} €
                            </text>
                        </div>
                        <div v-for="refound in refund_from" :key="refound.from" class="refound-rectangle to">
                            <text>{{ $t('Due_to') }} {{ refound.toUsername }} </text>
                            <text class="number">
                                {{ refound.amount}} €
                            </text>
                        </div>
                        <div v-for="refound in refund_others" :key="refound.from" class="refound-rectangle others">
                            <text>{{ $t('from') }} {{ refound.fromUsername }} {{ $t('to') }}
                                {{refound.toUsername}}</text>
                            <text class="number">
                                {{ refound.amount }} €
                            </text>
                        </div>
